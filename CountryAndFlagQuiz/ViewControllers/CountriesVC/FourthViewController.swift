...
class FourthViewController: UIViewController {
    ...
    func config() {
        ...
        countryCV.backgroundColor = UIColor(named: "Whiteish")
    ...
    func showFlagView(flag: String){
        ...
        UIApplication.shared.keyWindow!.addSubview(flagView)
        
        let flagFormat = flag.replacingOccurrences(of: ".", with: "")
        let image = UIImage(named: "\(flagFormat).png")
        bigFlag.image = image?.rotate(radians: .pi * 1.5)
    ...
    func animate() {
        UIView.animate(withDuration: 0.6, delay: 0, usingSpringWithDamping: 0.9, initialSpringVelocity: 4, options: .curveEaseIn, animations: {
...
extension FourthViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if searching {
            showFlagView(flag: searchList[indexPath.row].flagUrl)
        } else {
